...
# Get data
data <- read.xlsx( "../source_data/fig2gh/20230222_FACS.xlsx", sheetIndex = 2) %>% 
...
blank <- data$G12
data <- data %>% select(-G12,-H12,-Time..s.,-Temp....C.)
...
data.plot <- data.melted %>% filter(name=="NLIM-PRO",time<13)
...
ggsave("../figures/fig2g_230222_growthcurve_NLIM_PRO.svg", width = 4, height = 3, bg='transparent')
...
df_long <- reshape(df.t_lag, 
                   ...
                   direction = "long") %>% rename(well=time,t_lag=value)
...
t_lags.plot <- t_lags %>% filter(type=="NLIM")
# Pull out mean of unsorted for graph ordering
t_lags.order <- t_lags.plot %>% filter(subpop=="UNSORTED") %>% 
  select(nitrogen,t_lag.mean) 
t_lags.plot <- t_lags.plot %>% inner_join(t_lags.order,by=c('nitrogen'='nitrogen')) %>%
  rename(order=t_lag.mean.y,t_lag.mean=t_lag.mean.x)
...
ggsave("../figures/fig2h_230222_t_lag_NLIM.svg", width = 4, height = 3, bg='transparent')
