...
CLS.data <- read.xlsx(file = "../source_data/fig2d/CLS_measurements_R.xlsx" ,2, header=TRUE) %>%
  mutate(
...
CLS.data <- CLS.data %>%
  select(nitrogen,type,Condition,Survival,Replicate,Subpop,Time_d) %>% 
...
CLS.data.amino_acid <- CLS.data %>% filter(nitrogen=='Urea',type=='NREP') # Change here for other conditions
...
ggsave("../figures/fig2e_CLS_subpop_time_NREP_THR.svg", width = 4, height = 3, bg='transparent')

# CLS bar chart for subpop after 20d (why not 30d ?)

CLS.data.time <- CLS.data %>% filter(Time_d==30,type=="NLIM")

# Pull out mean of unsorted for graph ordering
CLS.order <- CLS.data.time %>% filter(Subpop=="UNSORTED") %>% 
  select(nitrogen,survival.mean) 
CLS.data.time <- CLS.data.time %>% inner_join(CLS.order,by=c('nitrogen'='nitrogen')) %>%
  rename(order=survival.mean.y,survival.mean=survival.mean.x)
...
ggsave("../figures/fig2f_CLS_subpop_30d.svg", width = 4, height = 3, bg='transparent')
